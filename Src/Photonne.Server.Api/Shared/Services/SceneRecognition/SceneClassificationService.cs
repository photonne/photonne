...
public class SceneClassificationService
{
    ...
    private readonly ISceneRecognitionClient _client;
    private readonly SettingsService _settings;
    private readonly SceneRecognitionOptions _options;
    private readonly ILogger<SceneClassificationService> _logger;

    public SceneClassificationService(
        ApplicationDbContext dbContext,
        ISceneRecognitionClient client,
        SettingsService settings,
        IOptions<SceneRecognitionOptions> options,
    ...
    public async Task<int> ClassifyAndStoreAsync(Guid assetId, CancellationToken cancellationToken)
    {
        ...
        // correct semantics — and matches how ObjectDetections behave.
        var existing = await _dbContext.SceneClassifications
            .Where(s => s.AssetId == assetId)
            .ToListAsync(cancellationToken);
        if (existing.Count > 0)
        {
            _dbContext.SceneClassifications.RemoveRange(existing);
        }
        ...
        foreach (var s in response.Scenes.OrderBy(s => s.Rank))
        {
            ...
            _dbContext.SceneClassifications.Add(new SceneClassification
            {
                AssetId = assetId,
    ...
    /// SceneRecognition:Enabled is the static fallback / default).
    /// </summary>
